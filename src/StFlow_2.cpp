...
void StFlow::resize(size_t ncomponents, size_t points)
{
    ...
    else if (m_transport_option ==  c_Lewis1_Transport) {
        m_diff.resize(m_nsp*m_points);
    } 
    else {
        m_multidiff.resize(m_nsp*m_nsp*m_points);
...
void StFlow::setTransport(Transport& trans, bool withSoret)
{
    ...
    if (m_trans->model() == cMulticomponent) {
     ...
     else if (m_trans->model() == cLewis1Transport) {
        m_transport_option = c_Lewis1_Transport;
        m_diff.resize(m_nsp*m_points);
     }
else {
                        ...
                        doublereal* rg, integer* diagg, doublereal rdt)
{
    ...
    for (j = jmin; j <= jmax; j++) {
        ...
        //------------------------------------------
            ...
            //    informatio m_lambda/(rho*spec_heat)n to the left (j+1 -> j) from the
